...
def forward(pi, A, B, O):
  ...
  for t in range(1,N):
    for i in range(S):
      alpha[i,t] = B[i,O[t]] * np.dot(A[:,i].T, alpha[:,t-1])
...
def backward(pi, A, B, O):
  ...
  for t in reversed(range(N - 1)):
    for j in range(S):
      beta[j,t] = np.dot(A[j,:].T, (B[:,O[t+1]] * beta[:,t+1]))
...
def viterbi(pi, A, B, O):
  ...
  for t in range(1, N):
    for i in range(S):
      delta[i, t] = B[i, O[t]] * max(A[:,i] * delta[:,t-1])
      d[i,t] = np.argmax(max(A[:,i] * delta[:,t-1]))

  path.append(int(np.argmax(delta[:,N-1])))
  for i in reversed(range(N - 1)):
    path.append(int(d[path[-1],i]))
  path = reversed(path)
