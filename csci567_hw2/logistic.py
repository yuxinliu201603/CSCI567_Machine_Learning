...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    """
    temp_X = np.hstack([np.ones([N, 1]), X])
    temp_w = np.hstack([b, w])

    for i in range(max_iterations):
        temp_yyy = sigmoid(np.dot(temp_X, temp_w)) - y
        temp_y = (step_size / N) * temp_yyy
        temp_yy = np.expand_dims(temp_y, axis=-1)

        temp = np.sum(temp_yy * temp_X, axis=0)
        temp_w = temp_w - temp

    b = temp_w.transpose()[0]
    w = temp_w.transpose()[1:]
    assert w.shape == (D,)
    return w, b
...
def binary_predict(X, w, b):
    ...
    re = np.dot(temp_X, temp_w)
    preds = (re > 0).astype(int)
...
def multinomial_train(X, y, C,
    ...
    """
    temp_X = np.hstack([np.ones([N, 1]), X])
    temp_w = np.hstack([np.expand_dims(b, axis = -1), w])

    for i in range(max_iterations):
        over_flow_x = np.dot(temp_w, temp_X.T)
        over_flow_max = np.amax(over_flow_x, axis = 0)
        temp_matrix = np.exp(over_flow_x - over_flow_max)
        temp_sum = np.sum(temp_matrix, axis = 0)
        temp_matrix1 = (temp_matrix / temp_sum).transpose()
        temp_matrix1[np.arange(N), y] -= 1
        temp_w = temp_w - step_size*(np.dot(temp_matrix1.T, temp_X))
...
def multinomial_predict(X, w, b):
    ...
    TODO: add your code here
    """
    b = np.transpose(np.expand_dims(b, axis = 0))
    temp_X = np.hstack([np.ones([N, 1]), X])
    temp_w = np.concatenate((b, w), axis=-1)
    re = np.dot(temp_X, temp_w.transpose())

    preds = np.argmax(re, axis=-1)
    assert preds.shape == (N,)
    return preds
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in np.unique(y):
        ### relabel trainY with class i as 1, all others as 0 ###
        temp_y = (y == i).astype(int)
        ### train binary classifier and get wi ###
        wi, bi = binary_train(X, temp_y, w0=None, b0=None, step_size=0.5, max_iterations=1000)
        w = np.concatenate((w, np.mat(wi)), axis=0)
        b = np.concatenate((b, np.mat(bi)), axis=0)

    w = w[1:]
    b = np.array(b[1:]).ravel()
