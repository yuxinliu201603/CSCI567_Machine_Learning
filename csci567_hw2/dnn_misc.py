...
class linear_layer:
    ...
    def forward(self, X):
        ...
        #Apply XW + b to X
        temp_forward = np.dot(X, self.params['W'])
        forward_output = temp_forward + np.tile(self.params['b'], (N, 1))
...
class relu:
    ...
    def forward(self, X):
        ...
        ################################################################################
        temp_forward = (X > 0).astype(int)
        forward_output = temp_forward * X
    ...
    def backward(self, X, grad):
        ...
        backward_output = (X > 0).astype(int) * grad
        return backward_output
